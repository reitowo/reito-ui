...
const storybook = import.meta.env.VITE_STORYBOOK_URL || 'http://127.0.0.1:6007';
...
function WorkspacePreview() {
  ...
  return <div className="home-workspace">
    ...
    <div className="home-workspace-body">
      <aside className="home-workspace-nav" aria-label="示例组件入口"><span className="text-xs text-muted-foreground">工作面</span><a href="/?layer=complex&component=workspace"><Blocks className="size-4" />项目概览</a><a href="/?layer=complex&component=data-table"><Braces className="size-4" />数据与文件</a><a href="/?layer=ai&component=composer"><Sparkles className="size-4" />AI 助手</a><span className="mt-auto text-xs text-muted-foreground">Graphite / React</span></aside>
  ...
  return <TooltipProvider><div className="reito-root home-page">
    ...
    <main id="home-main">
      ...
      <section id="components" className="home-section home-container"><div className="home-section-heading"><span className="text-sm text-muted-foreground">组件体系</span><h2>从一个按钮，到完整工作面。</h2><p>按应用需求逐层组合，共享同一套主题、交互和设计约束。</p></div><div className="home-layer-grid">{[
        { title: '基础组件', english: 'Foundation', icon: Blocks, description: '表单输入、选择、导航与反馈。为日常交互建立一致的起点。', items: 'Button · Input · Select · Dialog', href: '/?layer=basic&component=button' },
        { title: '复杂组件', english: 'Workspace', icon: Braces, description: '数据表格、文件、编辑器与工作区。把多个操作组织成清晰流程。', items: 'DataTable · Form · Workspace', href: '/?layer=complex&component=data-table' },
        { title: 'AI 组件', english: 'Intelligence', icon: Sparkles, description: '消息、上下文、工具调用与产物。让应用接入自己的模型和执行服务。', items: 'Composer · ToolCall · ArtifactPanel', href: '/?layer=ai&component=composer' },
    ...
    <footer className="home-footer home-container"><a href="/" className="home-logo"><PanelLeft className="size-4" />Reito UI</a><span>Graphite · 为个人工具而构建</span><a href={repository} target="_blank" rel="noreferrer">GitHub<ExternalLink className="size-3" /></a></footer>
