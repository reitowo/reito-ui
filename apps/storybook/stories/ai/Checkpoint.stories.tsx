...
const meta = { id: "ai-checkpoint", title: "AI/Checkpoint 检查点", component: Checkpoint, args: { title: '布局调整前', description: '本地恢复点示例，不修改本机文件。', files: [{ path: 'workspace-settings.ts', status: 'modified' }, { path: 'layout.css', status: 'added' }], onRestore: fn(), confirmDescription: '这次恢复只调用本地示例回调，不会读写文件。' }, decorators: [Story => <StoryFrame><Story /></StoryFrame>], parameters: { docs: { description: { component: '检查点摘要、文件变更和确认对话框。onRestore 由宿主实现；等待时防止重复提交，失败保持对话框与检查点内容，成功关闭确认层并恢复焦点。组件不会修改磁盘。' } } } } satisfies Meta<typeof Checkpoint>;
export default meta;
type Story = StoryObj<typeof meta>;
export const Guidelines: Story = { name: "交互 · 恢复检查点", render: () => <CheckpointDemo /> };
...
export const RestoreFailureAndRetry: Story = { name: "交互 · 恢复失败与重试",
