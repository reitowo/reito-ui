...
const meta = { title: 'AI/MessageActions', component: MessageActions, args: { text: '只复制这一段消息正文。', onCopy: fn(), onRetry: fn(), onEdit: fn(), onFeedbackChange: fn() }, decorators: [Story => <StoryFrame><Story /></StoryFrame>], parameters: { docs: { description: { component: '独立消息操作条。复制回调接收 text 属性，不读取 DOM。重试、编辑、反馈由宿主提供；异步失败保留操作上下文，可重新尝试。' } } } } satisfies Meta<typeof MessageActions>;
export default meta;
type Story = StoryObj<typeof meta>;
export const Guidelines: Story = { name: '交互场景：消息与操作条', render: () => <MessageActionsDemo /> };
export const CopyAndFeedback: Story = { name: '交互场景：复制、反馈与编辑',
...
export const RetryError: Story = { name: '交互场景：重试失败与恢复',
  play: async ({ canvasElement, args }) => {
...
export const Disabled: Story = { name: '禁用', args: { disabled: true, feedback: 'down' } };
