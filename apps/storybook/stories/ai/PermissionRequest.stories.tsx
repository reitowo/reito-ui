...
const meta = { id: "ai-permissionrequest", title: "AI/PermissionRequest 权限请求", component: PermissionRequest, args: { title: '运行本地检查', description: '此处只展示授权状态，不会执行命令。', decision: 'pending', onDecision: fn() }, decorators: [Story => <StoryFrame><Story /></StoryFrame>] } satisfies Meta<typeof PermissionRequest>;
export default meta;
...
export const LocalDecision: Story = { name: "交互 · 允许与拒绝", render: () => <PermissionRequestDemo />, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole('button', { name: '拒绝' })); await expect(canvas.getByRole('status')).toHaveTextContent('已拒绝'); await expect(canvas.queryByRole('button', { name: '允许一次' })).not.toBeInTheDocument(); await userEvent.click(canvas.getByRole('button', { name: '重置权限示例' })); await userEvent.click(canvas.getByRole('button', { name: '允许一次' })); await expect(canvas.getByRole('status')).toHaveTextContent('已允许'); } };
