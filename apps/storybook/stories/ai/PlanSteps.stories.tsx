...
const meta = { title: 'AI/PlanSteps', component: PlanSteps, args: { steps: [{ id: 'read', title: '检查共享令牌', status: 'success' }, { id: 'build', title: '组合组件', status: 'running' }, { id: 'test', title: '验证状态', status: 'pending' }] }, decorators: [Story => <StoryFrame><Story /></StoryFrame>] } satisfies Meta<typeof PlanSteps>;
export default meta;
type Story = StoryObj<typeof meta>;
export const Guidelines: Story = { name: '计划进度场景',};
export const Selectable: Story = { name: '交互场景：选择步骤', render: () => <PlanStepsDemo /> };
export const Error: Story = { name: '失败', args: { steps: [{ id: 'check', title: '检查组件', description: '示例错误：需要补充一个可访问名称。', status: 'error' }] } };
