...
const meta = { title: 'AI/PromptSuggestions', component: PromptSuggestions, args: { items: [{ id: 'review', label: '检查共享样式', prompt: '检查侧栏布局与焦点样式' }], onSelect: fn() }, decorators: [Story => <StoryFrame><Story /></StoryFrame>] } satisfies Meta<typeof PromptSuggestions>;
export default meta;
type Story = StoryObj<typeof meta>;
export const Guidelines: Story = { name: '默认',};
export const SeedsEditableDraft: Story = { name: '交互场景：填入草稿后编辑发送', render: () => <PromptSuggestionsDemo />, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole('button', { name: '检查共享样式' })); const input = canvas.getByRole('textbox', { name: '消息草稿' }); await expect(input).toHaveValue('检查侧栏布局与焦点样式'); await expect(canvas.getByRole('status')).toHaveTextContent('建议只填入草稿'); await userEvent.type(input, '，并检查浅色主题'); await userEvent.click(canvas.getByRole('button', { name: '发送消息' })); await expect(canvas.getByRole('status')).toHaveTextContent('本地已记录：检查侧栏布局与焦点样式，并检查浅色主题'); } };
