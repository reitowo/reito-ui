...
type PlaygroundArgs = { label: string; value: string[]; query: string; minQueryLength: number; debounceMs: number; disabled: boolean; required: boolean; error: string; placeholder: string; emptyMessage: string; showSelectAll: boolean; allowCreate: boolean };
export const Playground: StoryObj<PlaygroundArgs> = { name: '参数调试', args: { label: '异步选择技术栈', value: ['react'], query: '', minQueryLength: 0, debounceMs: 120, disabled: false, required: false, error: '', placeholder: '输入关键词搜索…', emptyMessage: '没有匹配的选项', showSelectAll: true, allowCreate: true }, argTypes: { label: textControl, value: { control: 'check', options: ['react', 'typescript', 'tailwind', 'storybook', 'rust'] }, query: textControl, minQueryLength: numberControl, debounceMs: numberControl, disabled: booleanControl, required: booleanControl, error: textControl, placeholder: textControl, emptyMessage: textControl, showSelectAll: booleanControl, allowCreate: booleanControl }, parameters: { controls: { include: ['label', 'value', 'query', 'minQueryLength', 'debounceMs', 'disabled', 'required', 'error', 'placeholder', 'emptyMessage', 'showSelectAll', 'allowCreate'] } }, render: function Render({ allowCreate, ...args }) { const [, update] = useArgs(); return <AsyncMultiSelect {...args} error={args.error || undefined} selectedOptions={options.filter(option => args.value.includes(option.value))} loadOptions={loadOptions} onCreateOption={allowCreate ? label => ({ value: label.toLocaleLowerCase().replaceAll(' ', '-'), label, group: '自定义' }) : undefined} onQueryChange={query => update({ query })} onValueChange={value => update({ value })} className="max-w-md" />; } };
