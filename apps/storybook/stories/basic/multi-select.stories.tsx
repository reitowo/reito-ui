...
export const Playground: StoryObj<{ label: string; value: string[]; disabled: boolean; loading: boolean; error: string; placeholder: string; showSelectAll: boolean; allowCreate: boolean; grouped: boolean }> = {
  name: '参数调试', args: { label: '项目技术栈', value: ['react'], disabled: false, loading: false, error: '', placeholder: '搜索并选择…', showSelectAll: true, allowCreate: true, grouped: true },
  argTypes: { label: { control: 'text' }, value: { control: 'check', options: ['react', 'typescript', 'tailwind', 'storybook', 'rust'] }, disabled: { control: 'boolean' }, loading: { control: 'boolean' }, error: { control: 'text' }, placeholder: { control: 'text' }, showSelectAll: { control: 'boolean' }, allowCreate: { control: 'boolean' }, grouped: { control: 'boolean' } },
  parameters: { controls: { include: ['label', 'value', 'disabled', 'loading', 'error', 'placeholder', 'showSelectAll', 'allowCreate', 'grouped'] } },
  render: function Render({ allowCreate, grouped, ...args }) { const [, updateArgs] = useArgs(); return <MultiSelect {...args} options={options.map(option => grouped ? option : { ...option, group: undefined })} onCreateOption={allowCreate ? label => ({ value: label.toLocaleLowerCase().replaceAll(' ', '-'), label }) : undefined} onValueChange={value => updateArgs({ value })} className="max-w-sm" />; },
};
