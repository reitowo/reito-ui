...
export const Playground: StoryObj<{ orientation: 'vertical' | 'horizontal'; rowCount: number }> = {
  ...
  render: args => args.orientation === 'vertical' ? <ScrollAreaDemo rowCount={args.rowCount} /> : <ScrollArea className="h-48 w-72 max-w-full rounded-lg border" aria-label="文件列表"><div className="flex w-max gap-4 p-[var(--rui-content-padding)] text-sm">{Array.from({ length: Math.max(1, Math.min(50, Math.floor(args.rowCount))) }, (_, index) => <p key={index}>component-{index + 1}.tsx</p>)}</div><ScrollBar orientation={args.orientation} /></ScrollArea>,
};
