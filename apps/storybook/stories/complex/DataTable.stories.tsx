...
type PlaygroundArgs = { manual: boolean; rowCount: number; pageIndex: number; query: string; columnFiltersEnabled: boolean; ownerFilter: string; manageColumns: boolean; resizableColumns: boolean; loading: boolean; selectable: boolean; pageSize: number; caption: string; searchPlaceholder: string; error: string; empty: boolean };
export const Playground: StoryObj<PlaygroundArgs> = {
 name: '参数调试', args: { manual: false, rowCount: 23, pageIndex: 0, query: '', columnFiltersEnabled: true, ownerFilter: '', manageColumns: false, resizableColumns: false, loading: false, selectable: true, pageSize: 5, caption: '本地任务', searchPlaceholder: '筛选所有列…', error: '', empty: false },
 argTypes: { manual: booleanControl, rowCount: { control: { type: 'number', min: 0 } }, pageIndex: { control: { type: 'number', min: 0 } }, query: textControl, columnFiltersEnabled: booleanControl, ownerFilter: choiceControl(['', 'Reito', 'Lin', 'Ming']), manageColumns: booleanControl, resizableColumns: booleanControl, loading: booleanControl, selectable: booleanControl, pageSize: choiceControl([3, 5, 10, 20]), caption: textControl, searchPlaceholder: textControl, error: textControl, empty: recipeControl(booleanControl, '传入空 data；列定义仍由调用方提供。') },
 parameters: { controls: { include: ['manual', 'rowCount', 'pageIndex', 'query', 'columnFiltersEnabled', 'ownerFilter', 'manageColumns', 'resizableColumns', 'loading', 'selectable', 'pageSize', 'caption', 'searchPlaceholder', 'error', 'empty'] } },
 render: function Render(args) { const [, update] = useArgs(); const records = args.manual ? remoteRecords.slice(args.pageIndex * args.pageSize, (args.pageIndex + 1) * args.pageSize) : demoRecords; const filters: DataTableColumnFiltersState = args.ownerFilter ? [{ id: 'owner', value: { kind: 'select', operator: 'in', values: [args.ownerFilter] } }] : []; return <DataTable data={args.empty ? [] : records} columns={demoColumns} getRowId={row => row.id} manual={args.manual} rowCount={args.manual ? args.rowCount : undefined} globalFilter={args.query} onGlobalFilterChange={query => update({ query })} filterDefinitions={args.columnFiltersEnabled ? [filterDefinitions[1]] : []} columnFilters={filters} onColumnFiltersChange={next => update({ ownerFilter: next[0]?.value.kind === 'select' ? next[0].value.values[0] ?? '' : '' })} manageColumns={args.manageColumns} resizableColumns={args.resizableColumns} columnLabels={{ name: '任务', owner: '负责人', status: '状态', updated: '更新日期' }} pagination={{ pageIndex: args.pageIndex, pageSize: args.pageSize }} onPaginationChange={pagination => update({ pageIndex: pagination.pageIndex, pageSize: pagination.pageSize })} loading={args.loading} selectable={args.selectable} caption={args.caption} searchPlaceholder={args.searchPlaceholder} error={args.error} />; },
};
