...
import { createDataTableViewSnapshot, DataTable, matchesDataTableColumnFilter, serializeDataTableColumnFilters, type DataTableColumnFilterDefinition, type DataTableColumnFiltersState, type DataTableEditableColumn, type DataTableEditCommit, type DataTableEditingState, type DataTableEditMode, type DataTableExportRequest, type DataTableSavedView } from '../../../../packages/ui/src/complex/index.js';
...
function ExportDataTable({ manual = false }: { manual?: boolean }) {
  ...
  const rows = manual ? remoteRecords.slice(0, 5) : remoteRecords;
  function exported(request: DataTableExportRequest<FilterRecord>) { setResult(`${request.scope} · ${request.format} · cols=${request.columns.length} · loaded=${request.loadedRows.length} · ids=${request.rowIds.length} · host=${request.requiresHostData}`); }
