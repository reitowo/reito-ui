...
export function App() {
  ...
  useLayoutEffect(() => {
    ...
    try { localStorage.setItem('reito-workbench-theme', theme); } catch { /* In-memory theme remains available. */ }
  }, [theme]);
  useEffect(() => {
  ...
  return <AppShell className="workbench-shell reito-root" header={<div className="flex flex-wrap items-center gap-2"><h1 className="text-sm font-medium">Reito Workbench</h1><span className="text-xs text-muted-foreground">本地示例</span><div className="ml-auto flex items-center gap-1"><Button variant="ghost" size="icon-sm" aria-label="切换工作台主题" title={theme === 'dark' ? '浅色主题' : '深色主题'} onClick={() => setTheme(current => current === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? <Sun aria-hidden="true" /> : <Moon aria-hidden="true" />}</Button></div></div>} sidebar={<div className="grid gap-[var(--rui-content-gap)]"><p className="hidden truncate text-xs text-muted-foreground @2xl:block" title={workspaceName}>{workspaceName}</p><nav aria-label="工作台导航" className="workbench-nav">{([{ id: 'agent', icon: MessageSquare }, { id: 'files', icon: Files }, { id: 'settings', icon: Settings2 }] as const).map(item => <Button key={item.id} variant={view === item.id ? 'secondary' : 'ghost'} size="sm" aria-current={view === item.id ? 'page' : undefined} onClick={() => navigate(item.id)}><item.icon aria-hidden="true" />{viewLabels[item.id]}</Button>)}</nav><p className="hidden text-xs leading-5 text-muted-foreground @2xl:block">共享组件组成的工作面。消息、文件差异和设置均保存在当前示例。</p></div>}>
