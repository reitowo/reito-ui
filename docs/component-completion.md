...
共 **103 个稳定验收/评估项**：88 个建设验收项、15 个后续评估项；**7 项完成，1 项进行中，95 项待完成**。这些项不是组件数量，单个组件的高级能力会分项验收。`FORM-01`–`FORM-04`、`VIRT-01`–`VIRT-03` 已完成，当前推进 `TREE-01`。阶段表示依赖顺序，不表示已承诺的发布日期；应用级与后续评估项需形成明确范围后再实施。
...
| TREE-01 | TreeView 基础：ARIA tree/treeitem、受控展开/选择、上下左右/Home/End 导航、禁用节点、焦点恢复；保留原 DisclosureTree 的原生导航定位 | — | 进行中 | 待关联 | 下一步补受控展开/选择、树键盘模型与焦点恢复 |
