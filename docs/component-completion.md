...
| DASHBOARD-RECIPE-01 | Dashboard 工作面配方：AppShell/Workspace/Sidebar/Table/Form/Search 的真实本地交互、偏好与窄布局；不作为单一原子组件计数 | SHELL-PRESET-01、SHELL-SEARCH-01、TABLE-01、FORM-01 | ✅ 完成 | 复杂-dashboardrecipe-工作面配方--playground / --default / --narrow / --persisted-preferences / --empty-tasks / --loading-tasks / --error-tasks / --interactive；共 8 个应用配方 Story | [验收](validation-dashboard-recipe.md)：12 项新增、84 项 DataTable 回归、32 次 Story 组合、同页 Controls、四主题密度窄屏；check/build 通过；[配方](recipes/dashboard-workspace.md) |
| CONTENT-RECIPE-01 | Content 工作面配方：文档列表/导航/阅读/相关操作统一组合、空/加载/失败与视图状态；专用阅读器另行验收 | CONTENT-NAV-01、RESOURCE-02 | ✅ 完成 | 复杂-contentrecipe-内容工作面配方--playground / --default / --narrow / --grid-library / --empty-library / --loading-library / --error-library / --empty-document / --loading-document / --error-document / --interactive；共 11 个应用配方 Story | [验收](validation-content-recipe.md)：13 项新增、82 项相关回归、44 次 Story 组合、同页 Controls、四主题密度窄屏；check/build 通过；[配方](recipes/content-workspace.md) |
...
| AI-OVERLAY-01 | ChatOverlay 组合：Dialog/Conversation/Composer 复用、开关与焦点恢复、停止/错误/重试/空态演示；请求仍由宿主控制 | AI-PARTS-01 | ✅ 完成 | ai-chatoverlay--playground；7 个 Story | [验收](validation-chat-overlay.md)：8 项专项、18 项弹层回归、28 组 Story、同页 Controls、小窗口；check/build 通过 |
| AI-PALETTE-01 | ChatPalette 组合：紧凑弹层、命令/会话切换、快捷键边界、焦点、停止与错误恢复；不重造消息组件 | AI-OVERLAY-01、SHELL-SEARCH-01 | ✅ 完成 | ai-chatpalette--playground；7 个 Story | [验收](validation-chat-palette.md)：14 项专项/回归、28 组常规与 28 组打开审计；check/build 通过 |
| AI-VIRTUAL-01 | Conversation 长列表：动态高度、流式 part 更新、历史向前加载、回到底部、用户暂停跟随、焦点/选择保持；用真实增长的示例验收 | VIRT-02、AI-PARTS-01 | ✅ 完成 | ai-virtualconversation--playground；6 个 Story | [验收](validation-virtual-conversation.md)：19 项交互/回归、24 组主题密度、同页 Controls、窄屏；check/build 通过 |
...
这 15 项已完成书面采用/延期/不采用评估；评估状态独立于建设交付。有明确场景才转入建设阶段，ScrollTop 的独立建设项 BASIC-SCROLLTOP-01 已验收完成。PRO roadmap 行用于追踪可用性，不能以名称、宣传、计划日期或其他框架实现作为已发布证据。与阶段 7 的能力重叠不另计一次组件交付。
...
| EVAL-SCROLLTOP-01 | ScrollTop：评估长内容定位需求与现有滚动容器/返回底部区别，明确触发阈值与焦点行为 | CONTENT-RECIPE-01 | ✅ 评估完成 | 采用；见 BASIC-SCROLLTOP-01 | [逐项决策、来源与重启条件](research/remaining-component-decisions-2026-09-09.md)；评估完成不等于组件交付 |
...
| ID | 验收项 / 边界 | 依赖 | 状态 | Story | 证据 |
| --- | --- | --- | --- | --- | --- |
| BASIC-SCROLLTOP-01 | ScrollTop：元素/窗口目标、阈值、减少运动、焦点和同页 Controls | EVAL-SCROLLTOP-01 | ✅ 完成 | 基础-scrolltop--playground；6 个 Story | [验收](validation-scroll-top.md)：9 项交互、24 组主题密度审计；check/build 通过；[用法](components/scroll-top.md) |
