...
| [`RichTextEditor`](../packages/ui/src/complex/rich-text-editor.tsx) | Tiptap schema 持有真实文档树；`format` 明确 JSON/HTML/Markdown 输入输出，`value / onValueChange` 可受控并提供三格式 snapshot。内置可裁剪的紧凑工具栏，marks、普通/任务列表、段落对齐和 Emoji 都执行真实事务；链接面恢复选区、校验协议，历史按钮与原生快捷键共用撤销栈，组合输入期间不抢占 `Mod-K`。任务列表可由 Markdown 往返；对齐只由 JSON/HTML 保留；Emoji 节点可导出为 shortcode，但当前 Markdown 解析不会恢复节点身份。严格拒绝无效 JSON，支持空、只读、禁用和宿主/解析错误。提及、块操作、媒体与 AI 接点按后续 EDIT 项组合。 |
