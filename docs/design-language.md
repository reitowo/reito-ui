...
Conversation、WorkspacePane 与 ScrollArea 的阅读滚动区使用同一 `rui-scroll-focus`：鼠标阅读和选择文字不加焦点装饰，键盘聚焦只在内侧显示一条焦点色标记，不把整块正文描成输入框。子按钮获得焦点时不连带点亮父面板；Tab、滚动键与按钮本身的焦点提示不变。系统强制颜色模式下以系统色内描边替代可能被移除的阴影标记。
