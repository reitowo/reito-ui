...
import { ConfirmPopover, DataTable, SettingsSection, SplitButton, UserInfo, WorkspacePane } from '@reito/ui/complex';
...
0.4 当前提供基础 70、复杂 36、AI 20 个组件族。基础层中的 50 族来自固定 CLI 生成，Toast、MultiSelect、NumberField、Meter、MeterGroup / ProgressGroup、Rating、Knob、InputTags、AsyncCombobox、AsyncMultiSelect、ColorPicker / ColorInput、InputDate、InputTime、DateTimePicker、PasswordInput、InputMask、Listbox、ImageGallery、Carousel 与 ImageCompare 是本库实现的 20 族；复杂层的 SplitButton、ConfirmPopover 与 UserInfo 组合用法见 [拆分按钮指南](components/split-button.md)、[锚点确认指南](components/confirm-popover.md)和[用户信息行指南](components/user-info.md)，AI 层的 MarkdownContent / RichMessage 同时覆盖静态与追加式流，用法见 [阅读渲染指南](components/markdown-content.md)。具体公开契约见 [组件分层](component-layering.md)。
