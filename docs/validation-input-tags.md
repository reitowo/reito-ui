...
2026-09-07，TAGS-01 已完成。新增 `InputTags` 基础组件族，支持任意字符串创建、中英文分隔键、可配置规范化、忽略/拒绝/允许重复、数量与字符上限、原位编辑、删除、受控/非受控值、只读/禁用和原生重复表单值。此处保留基础阶段证据；高级输入的最终状态见 [TAGS-02 验收](validation-input-tags-advanced.md)。
