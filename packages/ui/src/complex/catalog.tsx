...
export function RichTextEditorDemo() {
  ...
  return <div className="space-y-[var(--rui-content-gap)]"><RichTextEditor format="markdown" value={value} onValueChange={next => setValue(String(next))} label="组件说明" description="本地受控 Markdown；输入 / 插入结构，输入 @ 选择本地成员。" mentionItems={[{ id: 'reito', label: 'Reito', description: '设计系统维护者' }, { id: 'lin', label: 'Lin', description: '组件工程' }]} /><p role="status" className="text-xs text-muted-foreground">宿主值：{value.length} 个字符</p></div>;
}
...
export const complexCatalog: ComplexCatalogEntry[] = [
  ...
  { id: 'rich-text-editor', name: 'RichTextEditor 富文本编辑', description: 'Tiptap schema 文档模型，支持受控多格式内容、任务列表、对齐、Emoji、斜杠命令、提及、链接与历史。', component: RichTextEditorDemo },
