...
export function DataTablePreferences<TData>({ snapshot, columnIds, savedViews = [], activeViewId, exportScopes, onApplyView, onSaveView, onDeleteView, createExportRequest, onExport, disabled }: {
  ...
  return <div className="flex items-center gap-[var(--rui-content-gap-sm)]">
    {onExport && <Popover><PopoverTrigger render={<Button variant="outline" size="sm" disabled={disabled} />}><Download aria-hidden="true" />导出</PopoverTrigger><PopoverContent align="end" className="w-72 max-w-[calc(100vw-var(--rui-space-8))] gap-[var(--rui-content-gap)] p-[var(--rui-content-padding)]"><div><PopoverTitle>请求导出</PopoverTitle><PopoverDescription className="text-xs">组件声明范围；文件生成和下载由宿主完成。</PopoverDescription></div><div className="grid grid-cols-2 gap-[var(--rui-content-gap-sm)]"><SelectInput aria-label="导出范围" size="sm" value={scope} onValueChange={(nextValue) => setScope(nextValue as DataTableExportScope)} options={[
    ...exportScopes.map(value => ({
        value: value,
        label: value === 'current-page' ? '当前页' : value === 'filtered' ? '筛选结果' : value === 'selected' ? '已选记录' : '全部数据'
    }))
]}/><SelectInput aria-label="导出格式" size="sm" value={format} onValueChange={(nextValue) => setFormat(nextValue as DataTableExportFormat)} options={[
    {
        value: "csv",
        label: "CSV"
    },
...
]}/></div><Button size="sm" disabled={pending === 'export'} onClick={() => { void exportRows(); }}>{pending === 'export' ? '正在请求…' : '请求导出'}</Button>{error && <p role="alert" className="text-xs text-destructive">{error}</p>}</PopoverContent></Popover>}
