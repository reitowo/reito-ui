...
async function open(page: Page, globals = 'theme:dark;density:compact') { await page.goto(`http://127.0.0.1:6007/iframe.html?id=${prefix}--column-management&viewMode=story&globals=${globals}`); const table = page.getByRole('region', { name: '可管理任务' }); await expect(table).toBeVisible(); return table; }
...
test('pin select moves a column to the right sticky region', async ({ page }) => {
  const table = await open(page); await manager(page, table); await page.getByRole('combobox', { name: '负责人固定位置' }).selectOption('right'); await expect(table.getByRole('columnheader', { name: /负责人/ })).toHaveCSS('position', 'sticky'); const leaf = table.getByRole('row').nth(1).getByRole('columnheader'); await expect(leaf.last()).toContainText('负责人');
});
