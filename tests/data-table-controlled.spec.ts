...
async function open(page: Page, story = 'remote-controlled', globals = 'theme:dark;density:compact') { await page.goto(`http://127.0.0.1:6007/iframe.html?id=${prefix}--${story}&viewMode=story&globals=${globals}`); const table = page.getByRole('region', { name: '远程任务' }); await expect(table).toBeVisible(); return table; }
...
test('controlled page size recalculates page count', async ({ page }) => {
  const table = await open(page); await table.getByRole('combobox', { name: '每页记录数' }).selectOption('10'); await expect(table.getByText('第 1 / 3 页')).toBeVisible(); await expect(table.getByRole('row')).toHaveCount(11);
});
